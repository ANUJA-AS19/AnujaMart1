...
function openListingDialog(existing, onSaved) {
  ...
  bindForm(form, save, async (fd) => {
    ...
    if (isEdit) await api.updateProduct(existing.id, input);
...
export async function seller(ctx) {
  ...
  function listingsPanel() {
    ...
    return table(
      ...
      data.listings.map((p) => {
        ...
        edit.addEventListener('click', () => openListingDialog(p, reload));
  ...
  function draw() {
    ...
    newBtn.addEventListener('click', () => openListingDialog(null, reload));
